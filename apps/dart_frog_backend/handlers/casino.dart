...
Future<Response> _getCasinoStatus(RequestContext context, _AuthContext auth) async {
  ...
  try {
    ...
    final result = await _dbQuery(
      'SELECT coins, last_daily_claim, quests_done, total_spins, total_wins FROM casino_users WHERE user_id = @userId',
      substitutionValues: {'userId': userId},
    );
    if (result.isEmpty) {
      await _dbQuery(
        "INSERT INTO casino_users (user_id, coins, last_daily_claim, quests_done) VALUES (@userId, 100, NOW(), '{}')",
        substitutionValues: {'userId': userId},
      );
    ...
    if (!dailyClaimed) {
      ...
      await _dbQuery(
        "UPDATE casino_users SET coins = coins + 100, last_daily_claim = NOW(), quests_done = '{}' WHERE user_id = @userId",
        substitutionValues: {'userId': userId},
      );
      dailyClaimed = true;
      questsDone = [];
    }
    final updated = await _dbQuery(
      'SELECT coins FROM casino_users WHERE user_id = @userId',
      substitutionValues: {'userId': userId},
    );
...
Future<Response> _claimDailyCoins(RequestContext context, _AuthContext auth) async {
  ...
  try {
    final now = DateTime.now();
    final check = await _dbQuery(
      'SELECT last_daily_claim FROM casino_users WHERE user_id = @userId',
      substitutionValues: {'userId': userId},
    );
    if (check.isNotEmpty) {
    ...
    await _dbQuery(
      'INSERT INTO casino_users (user_id, coins, last_daily_claim) VALUES (@userId, 100, NOW()) '
      'ON CONFLICT (user_id) DO UPDATE SET coins = casino_users.coins + 100, last_daily_claim = NOW()',
      substitutionValues: {'userId': userId},
    );
...
Future<Response> _completeQuest(RequestContext context, _AuthContext auth) async {
  ...
  try {
    ...
    await _dbQuery(
      "INSERT INTO casino_users (user_id, coins, last_daily_claim, quests_done) VALUES (@userId, 0, NULL, '{}') ON CONFLICT (user_id) DO NOTHING",
      substitutionValues: {'userId': userId},
    );

    final check = await _dbQuery(
      'SELECT quests_done FROM casino_users WHERE user_id = @userId',
      substitutionValues: {'userId': userId},
    );
    if (check.isNotEmpty) {
    ...
    await _dbQuery(
      'UPDATE casino_users SET coins = coins + 25, quests_done = array_append(quests_done, @questId) WHERE user_id = @userId',
      substitutionValues: {'questId': questId, 'userId': userId},
    );
...
Future<Response> _casinoSpin(RequestContext context, _AuthContext auth) async {
  ...
  try {
    ...
    await _dbQuery(
      'UPDATE casino_users SET coins = coins - @bet + @win, total_spins = total_spins + 1 WHERE user_id = @userId',
      substitutionValues: {'bet': bet, 'win': win, 'userId': userId},
    );
